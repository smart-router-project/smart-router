...
try:
    ...
    # Keep direct script execution working for existing shell wrappers.
    from bench_dataset import (
        ConversationsMap,
        ConvId,
        GenConvArgs,
...
async def send_request(
    ...
    timeout_sec: int = 120,
    session_id: str | None = None, # test############333
) -> ServerResponse:
    payload = {
        "model": model,
        "messages": messages,
        "seed": 0,
        "temperature": 0.0,
        # "logprobs": True,      # test############333
        # "top_logprobs": 5,   # test############333
    }
    ...
    if session_id is not None:                               # test############333
        headers["X-Session-ID"] = session_id                       # test############333
        payload["session_params"] = {"session_id": session_id}          # test############333
    # logger.info("Sending request with headers: %s", headers)
...
def build_random_warmup_token_pool(
    tokenizer: AutoTokenizer,
    conversations: ConversationsMap,
    max_pool_tokens: int = 200000,
) -> list[int]:
    token_pool: list[int] = []
    ...
    for messages in conversations.values():
        for message in messages:
            ...
            if len(token_pool) >= max_pool_tokens:
                return token_pool[:max_pool_tokens]

    if not token_pool:
        raise ValueError("Unable to build token pool for random warmup")

    return token_pool


def generate_random_warmup_text(
    tokenizer: AutoTokenizer,
    token_pool: list[int],
    ...
    content = prefix
    current_tokens = get_token_count(tokenizer, content)

    while current_tokens < target_num_tokens:
        remaining_tokens = target_num_tokens - current_tokens
        sample_size = max(remaining_tokens, 16)
        sampled_tokens = random.choices(token_pool, k=sample_size)

        # Keep a textual prefix for easy debugging while randomizing the body.
        content += " " + tokenizer.decode(sampled_tokens)
        current_tokens = get_token_count(tokenizer, content)

    return content


def generate_random_warmup_conversations(
    tokenizer: AutoTokenizer,
    conversations: ConversationsMap,
) -> ConversationsMap:
    logger.info(
        "%sGenerating randomized warmup conversations (isolated from benchmark session IDs)%s",
        Color.PURPLE,
        Color.RESET,
    )

    token_pool = build_random_warmup_token_pool(tokenizer, conversations)
    warmup_conversations: ConversationsMap = {}

    for conv_id, messages in conversations.items():
        assert len(messages) > 0, f"Conversation {conv_id} has no messages"
        assert messages[0]["role"] == "user", (
            f"Failed on conversation ID {conv_id}, first message role should be user"
        )

        input_num_tokens = get_token_count(tokenizer, messages[0]["content"])

        output_num_tokens = 1
        if len(messages) > 1:
            assert messages[1]["role"] == "assistant", (
                f"Failed on conversation ID {conv_id}, second message role should be assistant"
            )
            output_num_tokens = get_token_count(tokenizer, messages[1]["content"])

        warmup_conv_id = f"WARMUP::{conv_id}"
        warmup_conversations[warmup_conv_id] = [
            {
                "role": "user",
                "content": generate_random_warmup_text(
                    tokenizer,
                    token_pool,
                    input_num_tokens,
                    f"[warmup user {conv_id}]",
                ),
            },
            {
                "role": "assistant",
                "content": generate_random_warmup_text(
                    ...
                    token_pool,
                    output_num_tokens,
                    f"[warmup assistant {conv_id}]",
                ),
            },
        ]

    return warmup_conversations
...
def feed_global_start_tokens(
    ...
    assert conversation_start_rate > 0
    ...
    for idx in range(total_conversations):
        ...
        if idx > 0:
            interval = np.random.exponential(1.0 / conversation_start_rate)
            if verbose:
                logger.info(
                    "Global conversation start limiter sleeping for %.3f seconds",
                    interval,
                )
            if stop_event.wait(timeout=interval):  # type: ignore
                break

        start_queue.put(True)


def wait_for_global_start_slot(
    start_queue: mp.Queue | None,
    stop_event: mp.Event,  # type: ignore
    client_id: int,
    conv_id: ConvId,
    verbose: bool = False,
    ...
    while True:
        ...
        try:
            start_queue.get(timeout=0.5)
            return True
        except queue.Empty:
            if verbose:
                logger.info(
                    "Client %d waiting for a global start slot for conversation ID %s",
                    client_id,
                    conv_id,
                )
...
async def client_main(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while task_queue_empty is False:
            ...
            if conv_id not in started_convs:
                if not wait_for_global_start_slot(
                    start_queue,
                    stop_event,
                    client_id,
                    conv_id,
                    ...
                    logger.info(
                        f"{Color.YELLOW}Client {client_id} stopped before starting "
...
def get_client_config(
    ...
    # Skip the first user turn only when the warmup used the benchmark data
    # itself. Random warmup runs are isolated from the benchmark conversations.
    skip_first_turn = args.warmup_step and args.warmup_mode == WarmupMode.DATASET
    ...
    if args.conversation_start_rate < 0:
        raise ValueError("Conversation start rate must be a non-negative number")
...
async def main_mp(
    ...
    start_queue: mp.Queue | None = None
    ...
    token_feeder_thread: threading.Thread | None = None
    if bench_args.conversation_start_rate > 0:
        start_queue = mp.Queue()
        logger.info(
            "%sEnabling global conversation start rate limiter: %.3f conv/sec%s",
            Color.PURPLE,
            bench_args.conversation_start_rate,
            Color.RESET,
        )
        token_feeder_thread = threading.Thread(
            target=feed_global_start_tokens,
            args=(
                start_queue,
                len(input_conv),
                bench_args.conversation_start_rate,
    ...
    if token_feeder_thread is not None:
        token_feeder_thread.join(timeout=1.0)
...
def process_statistics(
    ...
    excel_output_file: str | None = None,  # 新增
) -> None:
    ...
    for percent in warmup_percentages:
        ...
        # Successful requests in this warmup view
        successful_requests = len(df)

        # Aggregate token counts
        total_input_tokens = float(df["input_num_tokens"].sum())
        total_output_tokens = float(df["output_num_tokens"].sum())
        total_tokens = total_input_tokens + total_output_tokens

        # Throughputs
        if runtime_sec > 0:
            requests_per_sec = successful_requests / runtime_sec
            input_token_throughput = total_input_tokens / runtime_sec
            output_token_throughput = total_output_tokens / runtime_sec
            total_token_throughput = total_tokens / runtime_sec
        else:
            requests_per_sec = 0.0
            input_token_throughput = 0.0
        ...
        params = {
            ...
            "input_token_throughput_tok_per_s": input_token_throughput,
            "output_token_throughput_tok_per_s": output_token_throughput,
            "total_token_throughput_tok_per_s": total_token_throughput,
        }
...
async def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--warmup-mode",
        type=WarmupMode,
        choices=list(WarmupMode),
        default=WarmupMode.DATASET,
        help=(
            "Warmup data source when --warmup-step is enabled. "
            "'dataset' reuses the first benchmark turn and can preserve session/KV "
            "state into the measured run. "
            "'random' generates randomized first-turn warmup traffic with isolated "
            "conversation IDs so the measured benchmark starts without reused "
            "session KV cache."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="Expected global rate (Poisson process) for starting new "
        "conversations across all clients in conversations/sec. Applied only "
        "before the first request of each conversation. Set to 0 to disable "
        "global start staggering.",
    )
    parser.add_argument(
        "--max-retries",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Specify the Excel output file name (e.g. result.xlsx). "
             "If not set, a timestamped filename will be generated.",
    )
    ...
    bench_args = BenchmarkArgs(
        ...
        conversation_start_rate=args.conversation_start_rate,
    )
    ...
    if args.warmup_step:
        ...
        warmup_verify_output = args.verify_output

        if args.warmup_mode == WarmupMode.RANDOM:
            warmup_conversations = generate_random_warmup_conversations(
                tokenizer, conversations
            )
            warmup_verify_output = False
        ...
        warmup_client_args = client_args._replace(
            ...
            verify_output=warmup_verify_output,
        )
        ...
        logger.info(
            "%sWarmup start (mode=%s)%s",
            Color.PURPLE,
            args.warmup_mode,
            Color.RESET,
        )
        warmup_start_ns = time.perf_counter_ns()
        warmup_output_convs, _ = await main_mp(
            warmup_client_args,
            req_args,
            warmup_bench_args,
            tokenizer,
        ...
        if args.warmup_mode == WarmupMode.DATASET:
            conversations = warmup_output_convs
    ...
    process_statistics(
        ...
        excel_output_file=args.excel_output_file,  # 新增
    )
